...
output: 
  html_document:
      ...
      keep_md: yes
---      
#Through spider eyes: urbanization imposes declines in insect biomass through the loss of large-bodied prey
De Wolf Katrien, Verstraelen Nele, Rommel Henri, Vantieghem Pieter, Vanthournout Bram, Bonte Dries.

R-code for analysis
Version: 5 December 2024
corresponding author: katrien.dewolf@ugent.be or dewolf_katrien@hotmail.com ; orcid id: https://orcid.org/0000-0002-9255-913X
...
```{r abund-graph1, fig.dim=c(15,30)}
data.st.tot <- data.st %>%
  group_by(plotid, U_landscape, U_local, location) %>%
  summarise(abundance=sum(n_indiv, na.rm=T))
...
g <- ggplot_gtable(ggplot_build(graph1))  ## colouring the facets is based on the code provided online on the github issues page of tidyverse ggplot2: https://github.com/tidyverse/ggplot2/issues/2096 
...
```{r abund-graph2, , fig.dim=c(10,15)}
...
```{r abund-graph3, fig.dim=c(15,30)}
#16 orders
...
graph3 <- ggplot(rel_n, aes(x=U_local, y=rel_n_indiv, fill=order)) +
  ...
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=rel(0.5)))
g <- ggplot_gtable(ggplot_build(graph3))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(plotid.col27, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph4-, fig.dim=c(15,30)}
ord_n <- data.st %>%
  group_by(plotid, U_landscape, U_local, location, order) %>%
  summarise(abundance=sum(n_indiv, na.rm=T)) %>%
  ungroup()
...
graph4 <- ggplot(ord_n, aes(x=U_local, y=abundance, fill=order)) +
  ...
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=rel(0.5)))
g <- ggplot_gtable(ggplot_build(graph4))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(plotid.col27, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph5, fig.dim=c(15,15)}
rel_n_urb <- data.st %>%
...
g <- ggplot_gtable(ggplot_build(graph5))
stript <- which(grepl('strip-t', g$layout$name))
...
```{r abund-graph6, fig.dim=c(15,15)}
n_urb <- data.st %>%
  group_by(U_landscape, U_local, urb_cat, order) %>%
  summarise(abundance=sum(n_indiv, na.rm=T)) %>%
  ungroup()
...
graph7 <- ggplot(top3_rel_n, aes(x=U_local, y=rel_n_indiv, fill=order)) +
  ...
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, hjust=1, size=rel(0.5)))
g <- ggplot_gtable(ggplot_build(graph7))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(plotid.col27, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph8, fig.dim=c(15,30)}
...
graph8 <- ggplot(top3_ord_n, aes(x=U_local, y=abundance, fill=order)) +
  ...
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=rel(0.5)))
g <- ggplot_gtable(ggplot_build(graph8))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(plotid.col27, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph9, fig.dim=c(10,15)}
...
g <- ggplot_gtable(ggplot_build(graph9))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(urb.col3, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph10, fig.dim=c(10,15)}
...
graph10 <- ggplot(top3_n_urb, aes(x=U_local, y=abundance, fill=order)) +
  ...
  theme_bw() +
  theme(axis.text.x=element_text(size=rel(0.7)))
g <- ggplot_gtable(ggplot_build(graph10))
stript <- which(grepl('strip-t', g$layout$name))
fills <- scales::alpha(urb.col3, 0.30)
k <- 1
for (i in stript) {
...
```{r abund-graph11, fig.dim=c(15,30)}
### for certain orders the suborder is not known so indicated with 'ord'
...
g1 <- ggplot_gtable(ggplot_build(graph11))
strip <- which(grepl('strip-', g1$layout$name))
fills <- c(scales::alpha(plotid.col27, 0.30))
k <- 1
for (i in strip) {
...
```{r abund-graph12, fig.dim=c(15,30)}
...
graph12 <- ggplot(top3_sub_rel, aes(x=U_local, y=rel_n_indiv, fill=suborder)) +
  ...
  theme(legend.position="bottom", axis.text.x=element_text(angle=45, hjust=1, size=rel(0.7)),
        plot.title=element_text(size=rel(1)))
g1 <- ggplot_gtable(ggplot_build(graph12))
strip <- which(grepl('strip-', g1$layout$name))
fills <- c(scales::alpha(plotid.col27, 0.30))
k <- 1
for (i in strip) {
...
```{r abund-graph13, fig.dim=c(15,15)}
graph13<- ggplot(top3_sub, aes(x=U_local, y=abundance, fill=suborder)) +
  ...
  theme_bw()+
  theme(axis.text.x=element_text(size=rel(0.7)))
g1 <- ggplot_gtable(ggplot_build(graph13))
strip <- which(grepl('strip-', g1$layout$name))
fills <- c(scales::alpha(urb.col3, 0.30))
k <- 1
for (i in strip) {
  j <- which(grepl('rect', g1$grobs[[i]]$grobs[[1]]$childrenOrder))
  g1$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
...
```{r abund-graph14, fig.dim=c(15,15)}
...
graph14 <- ggplot(top3_sub_rel_urb, aes(x=U_local, y=rel_n_indiv, fill=suborder)) +
  ...
  theme_bw() +
  theme(axis.text.x=element_text(size=rel(0.7)))
g1 <- ggplot_gtable(ggplot_build(graph14))
strip <- which(grepl('strip-', g1$layout$name))
fills <- c(scales::alpha(urb.col3, 0.30))
k <- 1
for (i in strip) {
...
combination of arthropod size class with taxonomic identification
we have size classes (SC) every 0.5 mm of body length.
SC01 : 0-0.5
SC02 : 0.5-1
....

**graph15: the body length (mm) of the collected arthropods**  
```{r bodysize-graph15, fig.dim=c(15,30)}
...
g1 <- ggplot_gtable(ggplot_build(graph15))
...
```{r func-sp-expl2, fig.dim=c(15,30)}
...
graph16<-data.st %>%
  ...
  ylab("Abundance")+
  xlab("Urbanization level at local scale")
...
Generalised linear mixed model: glmmTMB(abundance ~ U_landscape+U_local+(1|plotid), data=df_N, family=nbinom2, REML=T)
...
## BIOMASS 
calculated biomass as : 
W = a * length^b : body size length in mm ;  a = 0.04 ; b = 2.26 
...
```{r STAT-mod-biomass, fig.dim=c(10,10)}
df_biomass <- data.st %>%
  group_by(location, plotid, U_landscape, U_local, urb_cat, day) %>%
...
```{r STAT-mod-area, fig.dim=c(10,10)}
df_area <- data.st %>%
...
## BODY SIZE 
To test if urbanization decreases average size of the captured arthropods (the potential prey of the spider)
General linear mixed model : glmmTMB(log(avg_length) ~ U_landscape+U_local+(1|plotid), data=bz, family=gaussian, REML=T)
fixed effects: U_landscape and U_local: Urbanization level at landscape and local spatial scale => LOW, MEDIUM, HIGH
random effect: unique landscape identifier: plotid: P01 - P27
gaussian error distribution
```{r STAT-bz1, fig.dim=c(25,15)}
bz <- data.st %>%
...
```{r biodiv-func-species, fig.dim=c(13,18)}
...
d1<-ggplot(df_div, aes(U_landscape, species_richness, fill= U_local)) +
  geom_boxplot(position=position_dodge(width=0.75))+
  geom_jitter(position=position_dodge(width=0.75), alpha=0.6, size=1.5, colour="gray3")+ 
...
d3<-ggplot(df_div, aes(U_landscape, evenness, fill= U_local)) +
  ...
  scale_fill_manual(values = urb.col3, name="Urbanization level at local scale") +
  ylab("Evenness")+
  xlab("Urbanization level at landscape scale")+
  theme_bw()+
  theme(axis.title.y=element_text(size=rel(1)))
d3
...
g_sha<-ggplot()+
  ...
  geom_pointrange(data=preds_mod_sha, 
                  aes(x=U_local, y=emmean, ymin=lower.CL, ymax=upper.CL, fill=U_local), position=position_dodge(width=0.5), size = 1, pch = 21)+
  scale_fill_manual(values=urb.col3, name="")+
...
focus on functional species of the top 3 genera: Diptera, Hemiptera and Hymenoptera (respectively 55%, 26% and 11% of all individuals)
#### NMDS: functional species (Diptera, Hemiptera, Hymenoptera) 
Non-metric multidimensional scaling (NMDS) ordination based on Bray-Curtis dissimilarities of the functional species. Only the functional species of the three most abundant orders (Diptera, Hemiptera, and Hymenoptera) were included. 
According to the first three NMDS ordination axis and final stress of 0.22. 
```{r bray-nmds-top3}
set.seed(4)
...
ord = metaMDS(dhh.matrix, autotransform=T, distance="bray", trymax=1000, k=3, expand=F, plot=F)
ord
stressplot(ord)
...
the effect of urbanization at both local and landscape scales on the functional arthropod  community composition was analysed by Permutational Multivariate Analysis of Variance (PERMANOVA) on the Bray-Curtis dissimilarity matrix.
permutations where restricted 
