...
**#Urbanization imposes declines in flying insect biomass through the loss of large-bodied species. De Wolf Katrien, Verstraelen Nele, Rommel Henri, Vantieghem Pieter, Vanthournout Bram, Bonte Dries.**

R-code for analysis Version: 07 May 2025 
corresponding author: [katrien.dewolf\@ugent.be](mailto:katrien.dewolf@ugent.be){.email} or [dewolf_katrien\@hotmail.com](mailto:dewolf_katrien@hotmail.com){.email} ; orcid id: <https://orcid.org/0000-0002-9255-913X>
...
plotid: unique landscape identifier. In total 27 landscapes are monitored P01 - P27
...
func_species: Functional taxa groups were created based on the combination of the taxonomic identification level and the size class (**Revision** we modify this column to func_taxa as we did not perform species level identification)
...
#sizeclass: categorical variable indicating the size class of each specimen: a different size class was assigned every 0.5 mm unit of body length.
#ord_sub: combination of order level and suborder level taxonomic identification
#func_species: Functional taxa groups were created based on the combination of the taxonomic identification level and the size class (**Revision** we modify this column to the name func_taxa as we did not perform species level identification)

#revision rename column func_species to func_taxa
data.st <- data.st %>%
  rename(func_taxa = func_species)
```
...
data.st$func_taxa <- as.factor(data.st$func_taxa)
...
#revision: grey scale palettes for figures
urb.col3grey <- c("#B1B1B1", "#666666", "#000000")

col_vec <- c(rep("#8B008B", 3), rep("#E6E600", 3), rep("#38A800", 3))
plotid.col27 <- rep(col_vec, 3) # for colouring the facets 

col_vecgrey <- c(rep("#000000", 3), rep("#666666", 3), rep("#B1B1B1", 3)) 
plotid.col27grey<- rep(col_vecgrey, 3)
...
add_colour_facets_plotid_col <- function (graph) {
  g <- ggplot_gtable(ggplot_build(graph))
  stript <- which(grepl('strip-t', g$layout$name))
  fills <- scales::alpha(plotid.col27, 0.50)
  k <- 1
  for (i in stript) {
...
add_colour_facets_plotid_grey <- function (graph) {
  g <- ggplot_gtable(ggplot_build(graph))
  stript <- which(grepl('strip-t', g$layout$name))
  fills <- scales::alpha(plotid.col27grey, 0.50)
  k <- 1
  for (i in stript) {
    j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
    g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
...
add_colour_facets_urb_col <- function(graph) {
  g <- ggplot_gtable(ggplot_build(graph))
  stript <- which(grepl('strip-t', g$layout$name))
  fills <- scales::alpha(urb.col3, 0.50)
  k <- 1
  for (i in stript) {
    j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
    g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
...
#
add_colour_facets_urb_grey <- function(graph) {
  g <- ggplot_gtable(ggplot_build(graph))
  stript <- which(grepl('strip-t', g$layout$name))
  fills <- scales::alpha(urb.col3grey, 0.50)
  k <- 1
  for (i in stript) {
    j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
    g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- fills[k]
...
We collected and measured a total of 9605 arthropods (average per 180 cm2 sticky trap per week ± SD: 119 ± 79; with min-max per trap: 15-487), mainly belonging to the orders Diptera, Hemiptera and Hymenoptera (respectively 55%, 26% and 11% of all individuals)
```{r exploration}
...
graph1 <- ggplot(data.st.tot, aes(x=U_local, y=abundance, fill=U_local)) +
  ...
  scale_fill_manual(values=urb.col3grey, guide="none") +
  facet_grid(~plotid) +
...
p1<-add_colour_facets_plotid_col(graph1)
p1
...
graph2 <- ggplot(data.st.tot, aes(U_landscape, abundance, fill=U_local)) +
  ...
  scale_fill_manual(values=urb.col3grey, name="Urbanization level \nat local scale") +
...
p3 <- add_colour_facets_plotid_col(graph3)
p3
...
p4 <- add_colour_facets_plotid_col(graph4)
p4
...
graph5 <- ggplot(rel_n_urb, aes(x=U_local, y=rel_n_indiv, fill=order)) +
  ...
  theme_bw()+
  theme(axis.text.x=element_text(size=rel(0.8)), plot.title=element_text(size=rel(0.9)))
  

p5 <- add_colour_facets_urb_col(graph5)
p5

ggsave("Figure_S2.pdf", plot=p5, height=12.5, width=17, units="cm")
```
...
p6 <- add_colour_facets_urb_col(graph6)
p6

#ggsave("graph6.pdf",plot=p6, height=12.5, width=17, units="cm")
```
...
p7 <- add_colour_facets_plotid_col(graph7)
p7
...
p8 <- add_colour_facets_plotid_col(graph8)
p8
...
p9 <- add_colour_facets_urb_col(graph9)
p9
...
p10 <- add_colour_facets_urb_col(graph10)
p10
...
p11 <- add_colour_facets_plotid_col(graph11) 
p11
...
p12 <- add_colour_facets_plotid_col(graph12)
p12
...
p13 <- add_colour_facets_urb_col(graph13)
p13
...
p14 <- add_colour_facets_urb_col(graph14)
p14

#ggsave("graph14.pdf", plot=p14 , height=15, width=15, units="cm")
```

## Functional taxa - body size distribution
...
p15 <- add_colour_facets_plotid_col(graph15)
p15
...
```{r func-taxa-expl1}
length(levels(data.st$func_taxa))
```
in total 197 functional species

### exploration functional species top 3 orders: DIPTERA-HYMENOPTERA-HEMIPTERA

**graphs16-17-18: functional species (the more green - yellow the larger in body size the functional species are)**
```{r func-sp-expl2}
...
graph16 <- data.st %>%
  ...
  droplevels() %>% 
  ggplot(aes(x=U_local, y=n_indiv, fill=func_taxa)) +
  geom_bar(stat="identity") +
  scale_fill_viridis_d() +
  labs(x="Urbanization level at local scale", y="Abundance") + 
...
g1<-ggplot() +
  ...
  scale_fill_manual(values=urb.col3grey, name="") +
  labs(x="Urbanization level at local scale", 
       y=bquote("Biomass (mg/" * 180*cm^2 * "/week)"))+
...
## graphs used for Figure 3 in article
...
#ggsave("Figure_S4.pdf", plot=p_avg_length, height=8, width=12, units="cm")
```
...
func.matrix <- data.st %>%
  group_by(location, plotid, U_landscape, U_local, urb_cat, day, func_taxa) %>%
  summarise(abundance=sum(n_indiv)) %>%
  spread(key=func_taxa, value=abundance, fill=0) %>%
...
species_richness
shannondiv <- vegan::diversity(func.matrix, index=c("shannon")) #Shannon Diversity
shannondiv
...
d1<-ggplot(df_div, aes(U_landscape, species_richness, fill=U_local)) +
  geom_boxplot(position=position_dodge(width=0.75)) +
  scale_fill_manual(values=urb.col3grey, name="Urbanization level at local scale") +
  labs(x="", y="Richness") +
  theme_bw()+
...
## Figure 4 in article
#ggsave("Figure_4.pdf",plot=fig_d, height=13, width=18, units="cm")
```
...
dhh.matrix <- data.st.dhh %>%
  dplyr::group_by(location, plotid, U_landscape, U_local, urb_cat, func_taxa) %>%
...
# customize the nmds plot: gg12$plot

#save the dataframes needed
df_ord_12 <- gg12$df_ord #ordination points
df_ellipse_12 <- gg12$df_ellipse #ordination ellipses
df_arrows_12 <- fit12$df_arrows #arrows of envfit

gg12.plot <- ggplot()+
  geom_point(data=df_ord_12, aes(x=x, y=y, color=Group, shape=Group), size=2)+
  scale_color_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"="#B1B1B1", "MEDIUM"="#666666", "HIGH"="#000000"))+
  scale_shape_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"=15, "MEDIUM"=16, "HIGH"=17))+
  geom_path(data = df_ellipse_12, aes(x = x, y = y, color = Group), show.legend = FALSE)+
  geom_segment(data=df_arrows_12, aes(x=0, xend=x, y=0, yend=y),
               arrow=arrow(length=unit(0.25, "cm")), col="gray30") +
  geom_text(data=df_arrows_12, aes(x=x, y=y, label=var), size=3, 
            position=position_nudge(x=-0.1, y=+0.055)) +
  xlim(x_limits)+
  xlab("NMDS1")+
  ylab("NMDS2")+
  customized_theme
...
#customize the nmds plot : gg13$plot

df_ord_13 <- gg13$df_ord #ordination points
df_ellipse_13 <- gg13$df_ellipse #ordination ellipses
df_arrows_13 <- fit13$df_arrows #arrows of envfit

gg13.plot <- ggplot()+
  geom_point(data=df_ord_13, aes(x=x, y=y, color=Group, shape=Group), size=2)+
  scale_color_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"="#B1B1B1", "MEDIUM"="#666666", "HIGH"="#000000"))+
  scale_shape_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"=15, "MEDIUM"=16, "HIGH"=17))+
  geom_path(data = df_ellipse_13, aes(x = x, y = y, color = Group), show.legend = FALSE)+
  geom_segment(data=df_arrows_13, aes(x=0, xend=x, y=0, yend=y),
               arrow=arrow(length=unit(0.25, "cm")), col="gray30")+
  geom_text(data=df_arrows_13, aes(x=x, y=y, label=var), size=3,
...
## Figure 2 in article
fig_2 <- ggarrange(gg12.plot, gg13.plot, common.legend=T, legend="bottom", nrow=2)
fig_2
#ggsave("Figure_2.pdf", plot=fig_2, height=18, width=12, units="cm")
```
...
pch_map <- c("LOW"=15, "MEDIUM"=16, "HIGH"=17)
shapes<- pch_map[as.character(U_local)]

plot(ord$point[,1], ord$point[,2], type="n", main="local scale", xlab="NMDS1", ylab="NMDS2", cex.axis=1.0)
points(ord, choices=c(1,2), col=colors, pch=shapes)
...
points(ord, choices=c(1,3), col=colors, pch=shapes)
...
pdf("Figure_S1_landscape_test2.pdf")
par(mfrow=c(1,2))
...
#colors <- c(rep("black", 9), rep("grey45",9), rep("grey85", 9), rep("black",9), rep("grey45",9), rep("grey85", 9), rep("black",9), rep("grey45",9), rep("grey85", 9) )

#colors <- c(rep("#000000", 9), rep("#666666",9), rep("#B1B1B1", 9), rep("#000000",9), rep("#666666",9), rep("#B1B1B1", 9), rep("#000000",9), rep("#666666",9), rep("#B1B1B1", 9) )

pch_map <- c("LOW"=15, "MEDIUM"=16, "HIGH"=17)
shapes<- pch_map[as.character(U_landscape)]
...
the effect of urbanization at both local and landscape scales on the functional arthropod community composition was analysed by Permutational Multivariate Analysis of Variance (PERMANOVA) on the Bray-Curtis dissimilarity matrix. permutations where restricted within plotid (cfr. random factor GLMM).
...
perman1<-adonis2(dhh.dist ~ U_landscape+U_local+U_landscape:U_local, data=info,
                 permutations=perm_plotid, by="terms")
...
#customize the nmds plot : bz_gg12$plot    
#save the dataframes needed
df_ord_bz_12 <- bz_gg12$df_ord #ordination points
df_ellipse_bz_12 <- bz_gg12$df_ellipse #ordination ellipses
df_arrows_bz_12 <- bz_fit12$df_arrows #arrows of envfit

bz_gg12.plot <- ggplot()+
  geom_point(data=df_ord_bz_12, aes(x=x, y=y, color=Group, shape=Group), size=2)+
  scale_color_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"="#B1B1B1", "MEDIUM"="#666666", "HIGH"="#000000"))+
  scale_shape_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"=15, "MEDIUM"=16, "HIGH"=17))+
  geom_path(data = df_ellipse_bz_12, aes(x = x, y = y, color = Group), show.legend = FALSE)+
  geom_segment(data=df_arrows_bz_12, aes(x=0, xend=x, y=0, yend=y),
               arrow=arrow(length=unit(0.25, "cm")), col="gray30") +
  geom_text(data=df_arrows_bz_12, aes(x=x, y=y, label=var), size=3.5, 
...
#customize the nmds plot : bz_gg13$plot 
#save the dataframes needed
df_ord_bz_13 <- bz_gg13$df_ord #ordination points
df_ellipse_bz_13 <- bz_gg13$df_ellipse #ordination ellipses
df_arrows_bz_13 <- bz_fit13$df_arrows #arrows of envfit

bz_gg13.plot <- ggplot()+
  geom_point(data=df_ord_bz_13, aes(x=x, y=y, color=Group, shape=Group), size=2)+
  scale_color_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"="#B1B1B1", "MEDIUM"="#666666", "HIGH"="#000000"))+
  scale_shape_manual(name="Urbanization level \nat local scale",
                     values=c("LOW"=15, "MEDIUM"=16, "HIGH"=17))+
  geom_path(data = df_ellipse_bz_13, aes(x = x, y = y, color = Group), show.legend = FALSE)+
  geom_segment(data=df_arrows_bz_13, aes(x=0, xend=x, y=0, yend=y),
               arrow=arrow(length=unit(0.25, "cm")), col="gray30") +
  geom_text(data=df_arrows_bz_13, aes(x=x, y=y, label=var), size=3.5, 
...
fig_S5 <- ggarrange(bz_gg12.plot, bz_gg13.plot, common.legend=T, legend="bottom", nrow=2)
fig_S5
#ggsave("Figure_S5.pdf", plot=fig_S5, height=18, width=12, units="cm")  #Supplemental Figure S5 

set.seed(4)
bz.dist<-vegdist(bz.matrix, method="bray") # bray-curtis dissimilarity matrix as input for adonis2
#set permutations via permute to test the effect of both local and landscape scale urbanization 
perm_plotid <- with(info, how(nperm = 9999, blocks = plotid))
perman_bz<-adonis2(bz.dist ~ U_landscape+U_local+U_landscape:U_local, method="bray", data=info, permutations=perm_plotid, by="terms")
perman_bz
```

this result clearly indicates that low urbanised local sites have larger insects (high size classes numbers: eg. SC16: body length from 7.5 - 8 mm; SC18: 8.5 - 9 mm, SC20: 9.5 - 10 mm)
